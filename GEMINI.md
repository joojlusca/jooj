...
## Constituição do Projeto (Protocolo VLAEG)

### 1. Esquemas de Dados (JSON Data Schemas)

#### Estado do Carrinho (Cosa Nostra Shop)
```json
{
  "cart": {
    "items": [
...
#### Estado do Agendamento (BarberFlow Booking)
```json
{
  "booking": {
    "step": 1,
    "service": "Corte" | "Barba" | "Combo" | null,
    "barber": "Enzo" | "Thiago" | "Vincenzo" | null,
...
#### Estado do Visualizador (ML Estética)
```json
{
  "activeService": "limpeza" | "drenagem" | "microagulhamento" | "led" | "ultrassom" | "massagens" | "sobrancelhas" | "lipedema"
}
```

### 2. Regras Comportamentais
*   **Controle de Estado:** O estado global da aplicação deve gerenciar o showroom de forma que apenas um mini-app interativo esteja ativo por vez.
*   **Transições de Visualização:** O painel do showroom deve transicionar suavemente com animações GSAP de desvanecimento e escala (`opacity` e `scale`) ao alternar de projeto.
*   **Responsividade do Showroom:** Em dispositivos mobile, os controles dos mini-apps devem se ajustar para empilhamento vertical simples.
*   **Comportamento de Hover:** Efeito magnético aplicado com GSAP no hover dos CTAs principais.

### 3. Invariantes Arquiteturais
*   **Tecnologia Core:** A aplicação é 100% Client-side React 19 executada no navegador.
*   **Folha de Estilo Única:** Todos os estilos customizados (ruído, efeitos de vidro) devem residir em `src/index.css`. Nenhuma injeção dinâmica de CSS inline que contorne o Tailwind.
*   **GSAP Cleanup:** Todos os efeitos ScrollTrigger ou tweens devem ser executados dentro de um contexto GSAP (`gsap.context()`) com cleanup adequado no `useEffect`.